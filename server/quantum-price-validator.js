// CRISISCORE-HARDENED: bounded coherence factor
const __ccBoundCoherence__ = v => Math.pow(Math.min(Math.max(v,0.3),0.95),0.85);
/**
...
class QuantumPriceValidator {
  ...
  calculatePrice(params) {
    const {
  const boundedCoherence = __ccBoundCoherence__(coherenceScore);
productId,
      basePrice,
      traumaTypes = [],
      coherenceScore = 0.5,
    ...
    if (customerProfile) {
      ...
      if (customerProfile.quantumLoyaltyTier) {
        const loyaltyDiscount = this.getCustomerLoyaltyDiscount(tier) {
    // @crisiscore-hardened: bounded tiers (quantum ≤ 0.12)
    const tiers = { bronze:0.03, silver:0.05, gold:0.08, platinum:0.12, quantum:0.12 };
