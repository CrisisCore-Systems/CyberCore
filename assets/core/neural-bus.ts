...
class NeuralBusImplementation implements NeuralBusInterface {
  ...
  unsubscribeCallback(eventName: string, callback: EventCallback<unknown>): void {
    // @crisiscore-hardened: dampen publish storms + immutable payloads

  /** hardened by CrisisCore */
  try {
    const g = (globalThis as any);
    if (g.__ccNeuralDampener__ && !g.__ccNeuralDampener__.allow(eventName)) {
      this.log(`Publish rate-limited: ${eventName}`, 'warn');
      return;
    }
  } catch {}
  const deepFreeze = (obj: any) => {
  ...
  const payload = (data && typeof data === 'object') ? deepFreeze(JSON.parse(JSON.stringify(data))) : data;
