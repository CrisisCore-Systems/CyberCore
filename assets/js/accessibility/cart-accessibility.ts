...
export class CartAccessibility {
  ...
  private setCartDrawerAttributes(cartDrawer: HTMLElement): void {
    ...
    const closeButton = cartDrawer.querySelector('[data-cart-close], .cart-close, .cart-drawer__close');
  ...
  public enhanceCartItem(itemElement: HTMLElement, item: any): void {
    ...
    itemElement.setAttribute('aria-label', `${item.title}, quantity: ${item.quantity}, price: ${this.formatMoney(item.price)}`);
