...
export class CartSystem {
  ...
  async #init() {
    try {
      ...
      domCache.initialize();
      domCache.setDebugMode(this.#config.debug);

      // Initialize accessibility features
      cartAccessibility.initialize({
        cartDrawerSelector: this.#config.selectors.cartDrawer,
      ...
      // Add listener for accessibility-triggered close request
      document.addEventListener('cart:request-close', () => {
  ...
  #attachEventHandlers(): void {
    ...
    const handleDOMReady = () => {
      // Cart toggle buttons - using DOM cache
      domCache.getElements(this.#config.selectors.cartDrawerToggle)
        .forEach(btn => {
      ...
      // Add to cart forms - using DOM cache
      domCache.getElements(this.#config.selectors.addToCartForm)
        .forEach(form => {
      ...
      // Register for DOM cache invalidation
      domCache.onInvalidate(() => {
        // When the DOM cache is invalidated (e.g., after cart update),
        // we need to refresh our cached cart elements
        this.#updateCartUI();
      });
  ...
  #updateCartUI(): void {
    ...
    // Update cart count - using DOM cache
    const countEl = domCache.getElement(this.#config.selectors.cartCount);
    if (countEl) {
      countEl.textContent = this.#cartData.item_count.toString();
      countEl.classList.toggle('hidden', this.#cartData.item_count === 0);
    }

    // Update cart total - using DOM cache
    const totalEl = domCache.getElement(this.#config.selectors.cartTotal);
    if (totalEl) {
    ...
    // Update cart items - using DOM cache
    const cartDrawer = domCache.getElement(this.#config.selectors.cartDrawer);
    if (cartDrawer) {
  ...
  #createCartItemElement(item: CartItem): HTMLElement {
    ...
    // Apply accessibility enhancements to the item
    cartAccessibility.enhanceCartItem(itemEl, item);
  ...
  public openCartDrawer(): void {
    const drawer = domCache.getElement(this.#config.selectors.cartDrawer);
    if (!drawer) return;
