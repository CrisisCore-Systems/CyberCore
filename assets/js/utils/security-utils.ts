...
export class SecurityUtils {
  ...
  private static readonly DANGEROUS_TAGS_PATTERN =
    /<(iframe|object|embed|form|input|button|textarea|select|option|applet|meta|link|style|base|body|html|script|frame|frameset|ilayer|layer|bgsound|title|head)\b[^>]*>/gi;
    ...
    Object.keys(attributes).forEach((attr) => {
      ...
      if (attr.startsWith('on') || attr === 'href' || attr === 'src' || attr === 'style') {
        if (attr === 'href' || attr === 'src') {
          ...
          const sanitizedStyle = attributes[attr].replace(
            /(expression|javascript|vbscript):/gi,
            ''
          );
