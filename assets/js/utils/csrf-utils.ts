...
export class CSRFUtils {
  ...
  private setCSRFCookie(token: string): void {
    ...
    document.cookie = `${this.CSRF_COOKIE_NAME}=${token}; expires=${expires.toUTCString()}; path=/; SameSite=Strict; Secure`;
  }
   ...
   */
  public appendToHeaders(headers: Headers | Record<string, string>): Headers | Record<string, string> {
    // Get CSRF token
    const token = this.getToken();
